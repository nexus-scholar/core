...
2. **Storage adapter**: PDF retrieval still needs a non-Laravel storage adapter only if core must run outside Laravel hosts.
...
1. **Decide non-Laravel storage scope**: Add a filesystem storage adapter only if core needs to run outside Laravel hosts.
...
- PDF retrieval now reuses existing successful files, rejects non-PDF or oversized downloads before storage, retries transient download failures, skips recently failed source URLs during cooldown, and writes deterministic portable PDF paths.
- Remaining work is optional non-Laravel storage support, optional broader source coverage, and release-level export hardening.
