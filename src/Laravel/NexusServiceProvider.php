...
final class NexusServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->singleton('nexus.provider_configs', function ($app) {
            $config = $app['config']->get('nexus.providers');

            return ProviderConfigRegistry::defaults(
                ieeeApiKey: $config['ieee']['api_key'] ?? null,
                s2ApiKey: $config['semantic_scholar']['api_key'] ?? null,
                pubmedApiKey: $config['pubmed']['api_key'] ?? null,
